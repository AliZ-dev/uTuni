...
class MouseWheel:
	def __init__(self):
		self.wheelup = False           		# if the mouse wheel scrolled up, i.e. WHEEL_UP event.
		self.wheeldown = False            	# if the mouse wheel scrolled down, i.e. WHEEL_DOWN event.
...
class Mouse:
	def __init__(self):
		...
		# AliZ
		self.scroll = MouseWheel()
		self.trajectory = [self.position, self.position]
...
class Internal:
	...
	def isMouseButton(self, theButton, theQuery):
		...
		# AliZ
			...
			aRet = theButton.wheelup
		elif theQuery == WHEEL_DOWN:
			aRet = theButton.wheeldown
	...
	def mouseWQ(self, theWindowName, theQuery):
		...
		# AliZ
		if theQuery == WHEEL_UP or theQuery == WHEEL_DOWN:
			aButton = self.getContext(theWindowName).mouse.scroll
	...
	def iarea(self, theX, theY, theWidth, theHeight):
		...
		if aMouseIsOver:
			if aMouse.anyButton.pressed:
				aRet = DOWN
			# AliZ
			elif aMouse.scroll.wheelup:
				aRet = WHEEL_UP
			elif aMouse.scroll.wheeldown:
				aRet = WHEEL_DOWN
			else:
				aRet = OVER
...
def _handleMouse(theEvent, theX, theY, theFlags, theContext):
	...
	# AliZ
	aEventWheel = 11 #cv2.EVENT_MOUSEWHEEL
	...
	# AliZ
	if theEvent == aEventWheel:
			#print(theFlags)
			if theFlags > 0:
				theContext.mouse.scroll.wheeldown = True
			#	theContext.mouse.scroll.wheelup = False
			else:
				theContext.mouse.scroll.wheelup = True
			#	theContext.mouse.scroll.wheeldown = False
	if theX != -1 and theY !=-1:
		theContext.mouse.trajectory[0] = theContext.mouse.trajectory[1]
		theContext.mouse.trajectory[1].x = theX
		theContext.mouse.trajectory[1].y = theY
	#print('DONE')
	theContext.mouse.position.x = theContext.mouse.trajectory[1].x
	theContext.mouse.position.y = theContext.mouse.trajectory[1].y
...
def update(theWindowName = ''):
	...
	# AliZ
	aContext.mouse.scroll.wheelup = False
	aContext.mouse.scroll.wheeldown = False
